...
def create_app(
    ...
    async def upload_top_scorers(response: Response, file: UploadFile = File(...)) -> dict:
        """Parse and persist the uploaded top scorers spreadsheet, returning its JSON form."""
...
async def _read_pdf_bytes(uploaded_file: UploadFile, max_size_bytes: int) -> bytes:
    ...
    return await _read_document_bytes(
        ...
        file_kind="PDF",
        type_description="a PDF",
    )


async def _read_excel_bytes(uploaded_file: UploadFile, max_size_bytes: int) -> bytes:
    """Ensure the provided file is an Excel spreadsheet and return its bytes respecting limits."""
...
async def _process_upload(
    ...
    resource_path: str,
    *,
    file_reader: Callable[[UploadFile, int], Awaitable[bytes]] = _read_pdf_bytes,
) -> dict:
    """Parse, persist and serialize an uploaded document, handling domain errors uniformly."""

    document_bytes = await file_reader(file, max_size_bytes)
...
async def _read_document_bytes(
    ...
    *,
    ...
    """Validate ``uploaded_file`` against ``allowed_content_types`` and return its bytes."""

    if uploaded_file.content_type not in allowed_content_types:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"The uploaded file must be {type_description}.",
        )
    ...
    if not file_bytes:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"The {file_kind} file is empty.",
        )

    if len(file_bytes) > max_size_bytes:
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            detail=f"The {file_kind} file exceeds the allowed size.",
        )
